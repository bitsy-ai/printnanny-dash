  ...
  <div class="divide-y divide-gray-200 lg:col-span-9 flex">
    ...
    >
      ...
      <div v-else class="pt-4 px-4 sm:p-4 w-full">
        <div>
            <h2 class="text-left text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">Device Information</h2>
        </div>
        <!-- Version -->
        <div class="w-full mx-auto divide-y-2 divide-gray-200">
        <dl class="mt-6 space-y-6 divide-y divide-gray-200">
          <Disclosure as="div" class="pt-6" v-slot="{ open }" :defaultOpen="true">
            <dt class="text-lg">
              <DisclosureButton class="flex w-full items-start justify-between text-left text-gray-400">
                <span class="font-medium text-gray-900">PrintNanny OS Version & Build</span>
            ...
            <DisclosurePanel as="dd" class="mt-2 pr-12">
              <pre class="text-base text-gray-500">{{ store.deviceInfo?.issue }}}</pre>
            </DisclosurePanel>
          </Disclosure>
        </dl>
        </div>
        ...
        <dl class="mt-6 space-y-6 divide-y divide-gray-200">
          <Disclosure as="div" class="pt-6" v-slot="{ open }" :defaultOpen="true">
            ...
            <DisclosurePanel as="dd" class="mt-2 pr-12">
              <pre class="text-base text-gray-500">{{ store.deviceInfo?.ifaddrs }}}</pre>
            </DisclosurePanel>
          </Disclosure>
        </dl>
        </div>
