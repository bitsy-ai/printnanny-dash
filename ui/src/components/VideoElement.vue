    ...
    <div class="md:w-1/3 sm:w-3/4 m-auto">
      <Listbox as="div" v-model="store.selectedStream">
        <ListboxLabel
          ...
          >Select a video stream:</ListboxLabel
        >
        <div class="relative mt-1">
          ...
          >
            <TextSpinner
              text="Loading stream list"
              v-if="!store.selectedStream"
            ></TextSpinner>
            <span class="block truncate" v-else>{{
              store.selectedStream.description
            }}</span>
      ...
      <div class="flex">
        <button
          ...
          :class="[
            store.status == ConnectionStatus.ConnectionReady
              ...
              : 'focus:ring-0 transition duration-150 ease-in-out pointer-events-none opacity-60',
            'block truncate',
          ]"
        ...
        <button
          ...
          :class="[
            store.status == ConnectionStatus.ConnectionStreamReady
              ? 'hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg '
              : 'focus:ring-0 transition duration-150 ease-in-out pointer-events-none opacity-60',
            'block truncate',
          ]"
