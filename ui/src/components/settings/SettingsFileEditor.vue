<template>
    <div class="divide-y divide-gray-200 lg:col-span-9 flex">
        <Transition name="fade" mode="out-in" :duration="{ enter: 800, leave: 500 }">
            ...
            <div v-else-if="selectedFile" class="divide-y divide-gray-200 pt-6 w-full">
                <div class="py-6 px-4 sm:p-6 lg:pb-8">
                    <h2 class="text-lg font-medium leading-6 text-gray-900 py-4">Edit Settings Files</h2>

                <Listbox as="div" v-model="selectedFile" class="md:w-3/4">
                    <ListboxLabel class="block text-sm font-medium text-gray-700">Select a file:</ListboxLabel>
                    ...
                    <ListboxButton class="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-3 pr-10 text-left shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm">
                        ...
                        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                            ...
                            <ListboxOption as="template" v-for="file in store.settingsFiles" :key="file.file_name" :value="file" v-slot="{ active, selected }">
                                <li :class="[active ? 'text-white bg-indigo-600' : 'text-gray-900', 'relative cursor-default select-none py-2 pl-3 pr-9']">
                                <span :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ file.file_name }}</span>
